...
class TSDFVolume:
  ...
  def __init__(self, vol_bnds, voxel_size, use_gpu=True):
    ...
    color = (256 * 256 * 128 + 256 * 128 + 128)
    self._color_vol_cpu = color*np.ones(self._vol_dim).astype(np.float32)
    ...
    if self.gpu_mode:
      ...
      self._cuda_src_mod = SourceModule("""
        __global__ void integrate(float * tsdf_vol,
          ...
          // Skip invalid depth
          float mask = mask_im[pixel_y*im_w+pixel_x];
          if (mask > 0)
              return;
          tsdf_vol[voxel_idx] = 1;
        }""")
...
def get_view_frustum(depth_im, cam_intr, cam_pose, max_depth):
